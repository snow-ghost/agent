...
// InstrumentedSolver wraps a Solver with telemetry
type InstrumentedSolver struct {
	*Solver
	telemetry *Telemetry
}

// NewInstrumentedSolver creates a solver with telemetry
func NewInstrumentedSolver(solver *Solver) *InstrumentedSolver {
	return &InstrumentedSolver{
		Solver:    solver,
		telemetry: NewTelemetry(),
	}
}

// Solve wraps the original Solve with telemetry
func (s *InstrumentedSolver) Solve(ctx context.Context, task core.Task) (core.Result, error) {
	start := time.Now()
	s.telemetry.LogTaskStart(ctx, task)

	// Call the original solve method
	result, err := s.Solver.Solve(ctx, task)

	duration := time.Since(start)
	// For now, use a simple iteration count - in a real implementation,
	// you'd need to instrument the evolution loop more carefully
	iterations := 1
	s.telemetry.LogTaskEnd(ctx, task, result, duration, iterations)

	return result, err
}

// GetTelemetry returns the telemetry instance
func (s *InstrumentedSolver) GetTelemetry() *Telemetry {
	return s.telemetry
}
